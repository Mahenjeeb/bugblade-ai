...
const editoreducer = (state, action) => {
  switch (action.type) {
    case "SET_LANGUAGE":
      const selectedConfig = state.editorConfig.find(
        (config) => config.name === action.payload
      );
      return {
        ...state,
        language: action.payload.toLowerCase(),
        code: selectedConfig ? selectedConfig.value : "",
        feedback: "",
      };
    case "SET_CODE":
    ...
    case "FIX_CODE_START":
      return { ...state, isLoading: true, error: null };
    case "FIX_CODE_SUCCESS":
      return {
        ...state,
        isLoading: false,
        fixCode: action.payload,
        error: null,
      };
    case "FIX_CODE_ERROR":
      return { ...state, isLoading: false, error: action.payload };
...
export const EditorContextProvider = ({ children }) => {
  ...
  const fixErrorWithAI = async (language, code) => {
    try {
      dispatch({ type: "FIX_CODE_START" });
      const prompt = `Review this ${language} code for bugs and output only the corrected code. No explanations, no comments, just the complete fixed code.\n${code}`;
      const response = await puter.ai.chat(prompt, { model: "gpt-5" });
      dispatch({
        type: "FIX_CODE_SUCCESS",
        payload: response.message.content,
      });
    } catch (error) {
      dispatch({ type: "FIX_CODE_ERROR", payload: error.message });
    }
  };
  const editorValue = {
    ...currentState,
